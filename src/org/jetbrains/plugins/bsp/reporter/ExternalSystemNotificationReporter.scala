...
import org.jetbrains.plugins.bsp.reporter.ExternalSystemNotificationReporter.{TaskDescriptor, convertFailure, throwableToException}
...
class ExternalSystemNotificationReporter(workingDir: String,
  ...
  override def finishCanceled(): Unit = {
    ...
    descriptors.foreach { case (id,task) =>
      ...
      finishTask(id, BspBundle.message("report.build.task.canceled"), result, time)
    }

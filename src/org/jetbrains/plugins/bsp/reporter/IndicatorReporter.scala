...
class IndicatorReporter(indicator: ProgressIndicator) extends BuildReporter {
  override def start(): Unit = {
    indicator.setText(BspBundle.message("report.build.running"))
  }
  ...
  override def finish(messages: BuildMessages): Unit = {
    ...
    if (messages.errors.isEmpty)
      indicator.setText(BspBundle.message("report.build.completed"))
    else
      indicator.setText(BspBundle.message("report.build.failed"))
  }

  override def finishWithFailure(err: Throwable): Unit = {
    indicator.setText(BspBundle.message("report.failed.with.message", err.getMessage))
  }

  override def finishCanceled(): Unit = {
    indicator.setText(BspBundle.message("report.canceled"))
  }


  override def warning(message: String, position: Option[FilePosition]): Unit = {
    indicator.setText(BspBundle.message("report.warning.with.message", message))
    indicator.setText2(positionString(position))
  }

  override def error(message: String, position: Option[FilePosition]): Unit = {
    indicator.setText(BspBundle.message("report.error.with.message", message))
    indicator.setText2(positionString(position))
  }

  override def log(message: String): Unit = {
    indicator.setText(BspBundle.message("report.building"))

...
import org.jetbrains.plugins.bsp.project.BspTask.{BspTarget, TextCollector}
import org.jetbrains.plugins.bsp.protocol.BspJob.CancelCheck
import org.jetbrains.plugins.bsp.protocol.session.BspSession.{BspServer, NotificationAggregator, ProcessLogger}
import org.jetbrains.plugins.bsp.protocol.{BspCommunication, BspJob, BspNotifications}
...
import org.jetbrains.plugins.bsp.reporter.{BuildMessages, BuildReporter, BuildToolWindowReporter, CompositeReporter, IndicatorReporter}
...
class BspTask[T](project: Project,
  ...
  override def run(indicator: ProgressIndicator): Unit = {
    implicit val reporter: BuildReporter = new CompositeReporter(
      ...
      // new CompilerEventReporter(project, CompilationId.generate()), // FIXME reporter extension point?
